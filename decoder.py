...
class Parser(object):
    ...
    def parse_sentence(self, words, pos):
        ...
        while state.buffer:
            ...
            predictions = self.model(torch.LongTensor([features]))
            possible_actions = [(self.output_labels[i], prob) for i, prob in enumerate(predictions.detach().numpy()[0])]
            ...
            for action in possible_actions:
                if action[0][0] == "shift":
                    if len(state.buffer) > 1 or len(state.stack) == 0:
                        valid_action = action
                        break
                elif action[0][0] == "left_arc":
                    if len(state.stack) > 0 and action[0][1] != "root":
                        valid_action = action
                        break
                elif action[0][0] == "right_arc":
                    if len(state.stack) > 0:
                        valid_action = action
                        break
            
            if valid_action:
                if valid_action[0][0] == "shift":
                    state.shift()
                elif valid_action[0][0] == "left_arc":
                    state.left_arc(valid_action[0][1])
                elif valid_action[0][0] == "right_arc":
                    state.right_arc(valid_action[0][1])
            else:
                break
